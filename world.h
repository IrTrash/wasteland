	...
	private :	
		...
		void pproc() //paint
		{
			...
			for(unsigned n=0;n<this->onum;n++)
			{
				...
				dbuf = objlist[n]->getdfirst(_pdata);
				if(dbuf == NULL)
				{
					continue;
				}
				...
				switch(objlist[n]->type)
				{
					...
					case _unit :
					{
						int x = (int)dbuf->getvfirst(_x), y = (int)dbuf->getvfirst(_y);
						pp.addfigure(_circle,x,y,dbuf->getvfirst(_size),RGB(255,255,255));
						dbuf2 = objlist[n]->getdfirst(_status);
						sprintf(str,"%d/%d",(int)dbuf2->getvfirst(_life) , (int)dbuf2->getvfirst(_hp));
						pp.addtext(str,x-25,y+10);
					}
					break;
				}
			}
		...
		bool iniobj(_type desttype, object *dest)
		{
			...
			switch(desttype)
			{
				...
				case _unit:
				{
					...
					dbuf.reset();
					dbuf.type = _status; 
					dbuf.add(_life, 50); //현재 생명 
					dbuf.add(_hp,50); //생명 최대치 
					dbuf.add(_boundtype, _circle);
					dbuf.add(_bounddelay, 0);
					dbuf.add(_boundlimit, 1);
					dest->adddata(&dbuf);
